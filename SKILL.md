---
name: dev-workflow
description: 软件工程全流程工作流（workspace 级别）。从需求文档出发，依次完成设计、审查、拆解、实现、验收、经验沉淀。支持多工程协作（业务工程、协议工程、配置工程）。工作目录为 workspace/server/，业务工程在 P1GoServer/ 子目录。
...
│
├── server_old/                ← Rust 遗留业务工程
│   └── ...                       大部分逻辑已重构到 P1GoServer
│
├── proto/old_proto/           ← 默认协议工程
│   ├── scene/
│   │   └── xxx.proto             协议定义源文件
│   ├── logic/
│   └── module.proto              模块定义
│
└── config/RawTables/          ← 默认配置工程
    ├── TownTask/                 任务配置表
    ├── TownNpc/                  NPC 配置表
    └── ...                       其他游戏配置表（Excel）
```
...
保存到 `P1GoServer/docs/design-[feature-name].md`，**等待用户确认后进入 Phase 3。**
...
保存到 `P1GoServer/docs/tasks-[feature-name].md`，**等待用户确认后进入 Phase 4。**
...
| 经验类型 | 目标文档 | 示例 |
|----------|----------|------|
| 编码规范问题 | `P1GoServer/.claude/rules/` | 命名规范、错误处理模式 |
...
| 架构约定 | `P1GoServer/CLAUDE.md` | 工程间协作规范、模块设计原则 |
...
| 重复出现的编码问题 | `P1GoServer/.claude/rules/` |
| 新的架构约定 | `P1GoServer/CLAUDE.md` |
...
- 所有设计文档和审查记录写入业务工程的 P1GoServer/docs/ 目录持久化
