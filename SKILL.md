...
你是一名软件工程流程编排专家。按照以下 7 个阶段有序推进，每个关键节点暂停等待用户确认后再继续。
...
1. **协议优先**：先完成协议工程，生成协议代码
2. **配置先行**：配置工程完成后，业务工程才能加载配置
3. **数据库就绪**：Migration 脚本在本地测试通过
...
4. **dev-workflow-test-designer**：评估测试覆盖是否充分
...
回顾整个流程，将经验教训沉淀为可复用的配置：

1. **沉淀到 Rules**：审查中反复出现的编码规范问题 → 更新 `.claude/rules/` 对应文件
2. **沉淀到 Agent Memory**：让各审查 Agent 把本次发现的模式记录到各自记忆中
3. **沉淀到 CLAUDE.md**：架构层面的新约定（如工程间协作规范）→ 追加到 CLAUDE.md
...
**问题**：协议工程有多个生成脚本，需要根据操作系统选择正确的脚本。
...
**现象**：构建时出现 proto 相关的编译错误（如 `undefined: proto.XxxMessage`）
