...
**问题**：如果测试文件放在独立目录（如 `.claude/tests/`），无法访问 `internal/` 下的业务代码。
...
| **集成测试**（跨模块） | `.claude/tests/` | 通过公开接口测试，不受限制 |
| **测试源文件**（备份） | `.claude/tests/` | 方便管理和复用 |
...
- 测试文件可以正常运行（位于正确目录）
- 不会被提交到代码仓库
- `.claude/tests/` 保留测试源文件作为备份

### 13.3 测试文件创建流程

```
1. 在 .claude/tests/ 创建测试源文件
